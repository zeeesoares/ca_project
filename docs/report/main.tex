...
High-Performance Computing (HPC) systems. Training such models requires long-running jobs over distributed infrastructures, 
where fault tolerance becomes essential. Checkpointing—periodically saving the model state—is the standard mechanism to ensure 
progress is not lost due to failures. However, as model sizes scale to billions of parameters, checkpoint operations generate 
massive I/O workloads.

In shared HPC environments, these workloads are typically directed to a Parallel File System (PFS), which is simultaneously 
accessed by multiple users and applications. This leads to contention, particularly when several jobs attempt to write checkpoints 
concurrently. The resulting "noisy neighbor" effect can degrade performance cluster-wide, increasing latency and even stalling 
training processes.

Existing approaches either optimize checkpoint size or leverage faster intermediate storage, but often lack coordination mechanisms 
to regulate global I/O pressure. This paper proposes a policy-driven, two-tier storage orchestration system that combines node-local 
SSD staging with centralized rate control. By decoupling checkpoint generation from persistent storage and introducing a Token 
Bucket-based migration strategy, the system provides predictable Quality of Service (QoS) while maintaining transparency to user 
applications.

We evaluate the proposed system on the Deucalion supercomputer, demonstrating its ability to reduce I/O contention and improve overall 
system stability during large-scale LLM training.
...
Checkpointing is a fundamental technique for fault tolerance in High-Performance Computing (HPC) applications. Since large-scale 
training jobs may execute for hours or even weeks, failures caused by hardware faults, network instability, or software errors can 
lead to significant computational losses. To mitigate this problem, applications periodically save their execution state to stable 
storage, enabling recovery from the most recent checkpoint instead of restarting from the beginning.

In distributed deep learning workloads, checkpoints typically include model parameters, optimizer states, gradients, and training 
metadata. The rapid growth of Large Language Models (LLMs) has significantly increased checkpoint sizes, often reaching tens or 
hundreds of gigabytes per save operation. As a result, checkpointing has evolved into a major source of I/O pressure in modern HPC 
systems.


\subsection{Parallel File Systems and Contention}

Parallel File Systems (PFSs) such as Lustre are designed to provide high aggregate throughput and scalable shared storage for HPC 
environments. These systems distribute data across multiple storage targets and metadata servers, enabling concurrent access from 
...
checkpoint operations are often triggered simultaneously across distributed jobs, generating sudden I/O spikes that overwhelm 
storage resources. This behavior increases latency, reduces effective throughput, and creates contention between unrelated 
applications sharing the same filesystem.

This phenomenon is commonly referred to as the "noisy neighbor" problem, where the I/O activity of a subset of jobs negatively 
impacts the performance of others. As LLM training scales further, these interference effects become increasingly problematic 
for shared HPC infrastructures.


\subsection{Motivation}

The main motivation of this work is that checkpointing is not latency-critical for persistence, but it is latency-critical for 
training continuation. 

In practice, training can resume as soon as checkpoint data is safely written to fast temporary storage, while durable persistence 
to the PFS can occur asynchronously.

This distinction enables the decoupling of checkpoint generation from long-term storage operations. By leveraging node-local SSDs 
as an intermediate staging layer and regulating background transfers to the PFS, it becomes possible to reduce contention, smooth 
I/O bursts, and provide predictable Quality of Service (QoS) across concurrent workloads.

The proposed approach therefore aims to preserve fast checkpoint completion from the application's perspective while improving 
overall storage stability at the cluster level.
...
The system is implemented as a user-space middleware layer, avoiding kernel modifications. The interception layer is realized using dynamic linking 
techniques.
